  ...
  async function handleEnter() {
    ...
    try {
      const { data, error: err } = await supabase
        .from('classes').select('name').eq('class_code', classCode).single();
      if (err || !data) { setError('반 서재를 찾을 수 없어요. 코드를 다시 확인해주세요.'); setLoading(false); return; }
      onEnter(classCode, data.name);
    } catch {
